...
module SchemaMapper
  class Schema
    attr_reader :type, :package, :url, :data_itemtype_url, :imports, :schema_imports
    ...
    def initialize(type, package="")
      ...
      @imports = get(:imports, '[]')
      @schema_imports = get(:schema_imports, '[]')
    ...
    def property_info(row)
      ...
      if DataType.needs_import?(type)
        ...
        if fully_qualified_java_class.blank?
          ...
          @schema_imports.push("#{@package}.#{type}")
        else
    ...
    def property_type(types)
      raise ArgumentError, 'types must be mappable' unless types.respond_to? :map
      return DataType.to_java(types[0].to_sym) unless types.size > 1
      return DataType.to_java(:Text) if types.include?('Text') # For 'Text or URL'
